...
App.TinyTownsRandomizer =
  ...
  randomize_all_player_buildings: ->
    ...
    App.TinyTownsRandomizer.set_player_buildings(x) for x in [1..6]
  ...
  randomize_player_buildings: (number) ->
    ...
    App.TinyTownsRandomizer.set_player_buildings (number)
  ...
  toggle_all_player_buildings: ->
    ...
    if HIDE_PLAYERS
      App.TinyTownsRandomizer.hide_player_buildings(x) for x in [1..6]
    else
      App.TinyTownsRandomizer.reveal_player_buildings(x) for x in [1..6]

  toggle_player_buildings: (number) ->
    $("#player-#{number} .purple").toggleClass("hidden")

  reveal_player_buildings: (number) ->
    $("#player-#{number} .purple").removeClass("hidden")

  hide_player_buildings: (number) ->
    $("#player-#{number} .purple").addClass("hidden")
  ...
  randomize_all: ->
    ...
    App.TinyTownsRandomizer.change_total_players(4)

  randomize_all_buildings: ->
    App.TinyTownsRandomizer.randomize_building(x) for x,y of BUILDINGS

  change_total_players: (number) ->
    TOTAL_PLAYERS = number
    App.TinyTownsRandomizer.toggle_player(x) for x in [1..6]
...
$(document).on "click", "[data-behavior~=randomize-player]", (event) ->
  ...
  App.TinyTownsRandomizer.randomize_player_buildings(number)

$(document).on "click", "[data-behavior~=reveal-player]", (event) ->
  event.preventDefault()
  number = this.getAttribute('data-number')
  App.TinyTownsRandomizer.toggle_player_buildings(number)

$(document).on "click", "[data-behavior~=reveal-all-players]", (event) ->
  event.preventDefault()
  App.TinyTownsRandomizer.toggle_all_player_buildings()
...
$(document).on "turbolinks:load", ->
  ...
  $('input[type=radio][name=total-players]').change ->
    number = this.value
    App.TinyTownsRandomizer.change_total_players(number)
